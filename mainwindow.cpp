...
void MainWindow::updatestatusBar()
{
    ...
    // 当前页面有错误时显示页面特定错误
    else if (ui->stackedWidget->currentIndex() == 0 && todayPageError)    message = "未成功查询在线人数";
    else if (ui->stackedWidget->currentIndex() == 1)
    {
        if (schedulePageCloudError && schedulePageCacheError) message = "未成功获取云端课程表，未成功获取缓存数据";
        else if (schedulePageCloudError) message = "未成功获取云端课程表";
        else if (schedulePageCacheError) message = "未成功获取缓存数据";
    }
    else if (ui->stackedWidget->currentIndex() == 3 && noticePageError) message = "未成功获取公告数据";
